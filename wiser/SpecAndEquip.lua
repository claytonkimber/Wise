	...
	if slot.specIndex then
		...
		if slot.specIndex ~= currentSpec then
			...
			if not canSwitch then
				print(
					"|cff00ccff[Wise]|r |cffff0000Cannot switch specs right now (cooldown, M+, or other restriction).|r"
				)
				return
			end
...
function Wise:CreateSpecEquipPropertiesPanel(panel, slotIndex, y)
	...
	if not isCurrentSpec then
		...
		note:SetText(
			"Switch to "
				.. (targetSpecName or "the selected spec")
				.. " first to choose a loadout.\nLoadouts are only available for your active spec."
		)
			...
			if #savedConfigs == 0 and C_ClassTalents.GetConfigIDsByClass then
				...
				for _, cid in ipairs(allConfigs) do
					...
					if
						...
						and info.name
						and info.name ~= ""
						and info.type == (Enum and Enum.TraitConfigType and Enum.TraitConfigType.Combat)
					then
