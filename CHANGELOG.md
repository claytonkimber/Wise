...
### Fixed
- **Override/vehicle replacement buttons now actually fire on unskinned vehicles** (e.g. the Gnarldor Isle war turtle). There are two kinds of vehicle and Blizzard routes them to different frames: a *skinned* one (the Xeronia drake, a Mechagon shredder) flips to `LE_ACTIONBAR_STATE_OVERRIDE` and puts its abilities on `OverrideActionBarButtonN`, while an *unskinned* one stays on `LE_ACTIONBAR_STATE_MAIN` and merely repages `MainActionBar` to `C_ActionBar.GetVehicleBarIndex()`, leaving them on the ordinary `ActionButtonN`. `[vehicleui]` is true for both, so the generated `/click` bound `OverrideActionBarButtonN` — a frame that is never mounted on an unskinned vehicle. The icons were correct throughout (the display path is pure page arithmetic and works for both), so the symptom was four right-looking buttons that did nothing. Routing is now `[overridebar]` → override bar, `[vehicleui]` → `ActionButtonN`, `[possessbar]` → `ActionButtonN`, all three emitted in one macro so the choice happens at click time and needs no rebind when the vehicle kind changes — which matters because rebinding is blocked in combat. Covered by `tests/vehicle_bar_click.lua` in the sim suite.
- **Special-bar slots no longer hide themselves on an unskinned vehicle.** The same `[overridebar]`-only assumption was baked into the slots' stored `conditions`, which is what the visibility driver is built from (`<conditions> show; hide`) — so on a turtle the buttons vanished entirely rather than merely failing to fire. Override slots now store `[overridebar][vehicleui]`, and existing saved slots are migrated on load. Possess slots deliberately keep plain `[possessbar]`: the two are paired `exclusive` states, and identical conditions would make the first win unconditionally.
- **Caller conditions are ANDed into each click route instead of concatenated.** `BuildSpecialBarClickMacro` put the caller's conditions *in front of* the route group (`[nooverridebar,novehicleui][overridebar]`). Multiple bracket groups are an OR, so the caller's own group stood alone as a complete clause — one that is true precisely when no bar is up. Combined with the exclusivity change below, every special-bar button became a no-op. Conditions now merge into each route's group (`[nooverridebar,novehicleui,overridebar]`), one group per caller-group × route pair. Positive routing tokens are also stripped from incoming conditions, so a route can no longer be duplicated (`[overridebar,…,overridebar]`) or contradicted (`[overridebar,…,possessbar]`, never true).
- **`exclusive` now suppresses the rest of the chain inside a graph slot too.** A slot's states compile either to discrete states (which call `ComputeEffectiveConditions`, so an exclusive `[overridebar]` state makes later states inherit `[nooverridebar]`) or — for graph slots — into a single macro whose lines were built from each node's *raw* condition, skipping that step entirely. So an exclusive override/vehicle node did not suppress the later `/cast` lines, and a graph slot fired and displayed the character's own spell while a vehicle bar was up. Graph lines now derive from the same `ComputeEffectiveConditions`, so exclusivity means the same thing in both shapes.
- **A graph slot on a vehicle now shows the vehicle ability's icon, not its own fallback's.** The icon path preferred the slot's *stored* icon — baked at compile time from the graph node's fallback spell — over live resolution, while the tooltip always resolved live. Hence "correct tooltip, wrong icon" on any multi-state slot. A live resolution that reaches a real override/vehicle/possess action now outranks the stored icon, in both the initial build and the dynamic refresh.
- **Possess-bar slots no longer show action bar 12's icons.** With no special bar up, `ResolveBarActionID` passes the raw id through — and the possess range 121-132 *is* action bar 12 (145-156 collide likewise). Every display path therefore read the player's own bar and painted a convincing but completely wrong icon, cooldown, usability tint, and tooltip onto a vehicle slot. Display now gates on the new `Wise:HasAnySpecialActionBar()` and treats the slot as empty otherwise. The override range 133-144 has no such collision, which is why only possess slots showed this.
- **Negating a multi-state conditional no longer produces a condition that still matches.** `Wise:NegateConditional` stripped only the outermost brackets before splitting on commas, so `[overridebar][vehicleui]` was read as one blob, only its first token was negated, and the result was `[nooverridebar][vehicleui]` — still true on a vehicle. Multiple bracket groups are an OR, so the negation is now an AND of every negated token in a single group. This is what makes an `exclusive` special-bar state correctly suppress a plain fallback (e.g. Dash) while a vehicle bar is up.
