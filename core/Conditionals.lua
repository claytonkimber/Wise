...
Wise.opieConditionals = {
	...
	-- Content state (ported from OPie)
...
function Wise:UpdateConditionalsTab()
	...
	-- Default subtab
	if not Wise.conditionalsSubTab then
		Wise.conditionalsSubTab = "builtin"
	end
...
function Wise:UpdateConditionalsValues()
	local container = Wise.OptionsFrame.Views.Conditionals.Content
	if not container or not container.rows or not container.conditionalsList then
		return
	end
