...
-- Custom-conditional engine, extracted to core/conditionals/ (ZoneAbility,
-- Vocabulary, ArgMatch, Predicates, Environment, EvalToken, ConditionString).
-- Those files load before this one and publish their internals on
-- Wise.CondEngine; the six names below are the only ones this file still needs.
--
-- Bound as upvalues rather than called through Wise.CondEngine.x or a Wise:
-- method because every one of them sits on a hot path — UpdateGroupDisplay runs
-- these per slot per refresh, and the per-call table lookup showed up in the
-- same GC/dispatch profiling that drove the closure hoisting elsewhere in this
-- file. Taken at load time, which is safe: the engine is fully populated by the
-- time GUI.lua runs.
local CondEngine = Wise.CondEngine
local CUSTOM_VIS_CONDITIONALS = CondEngine.CUSTOM_VIS_CONDITIONALS
local IsZoneAbilityActive = CondEngine.IsZoneAbilityActive
local GetZoneAbilitySpellButton = CondEngine.GetZoneAbilitySpellButton
local HasCustomConditionals = CondEngine.HasCustomConditionals
local EvalFullConditionString = CondEngine.EvalFullConditionString
local EvalConditionExact = CondEngine.EvalConditionExact
local EvalConditionString = CondEngine.EvalConditionString
...
-- Visual effects, extracted to core/effects/ (OverlayGlow, HoverIndication).
-- Both load before this file. The proc glow is reached through its Wise:Show/
-- HideOverlayGlow methods, but CreateGroupFrame drives hover state from the
-- ring frame's own OnEnter/OnLeave, so those three helpers plus the scale
-- constant are bound here as upvalues off Wise.Effects.
local Effects = Wise.Effects
local ShowHoverGlow = Effects.ShowHoverGlow
local HideHoverGlow = Effects.HideHoverGlow
local ApplyHoverScale = Effects.ApplyHoverScale
local HOVER_SCALE = Effects.HOVER_SCALE

-- Layout helpers extracted to core/layout/. ApplyLayout calls this once per
-- refresh for list-type interfaces, so bind it rather than reaching through
-- Wise.Layout on every call.
local ApplyListLayout = Wise.Layout.ApplyList
...
function Wise:ApplyLayout(frame, type, count, groupName)
	...
	elseif type == "list" then
		-- Vertical text-based list. The only layout that measures before it
		-- places (two passes over the buttons), so it lives in its own file --
		-- see core/layout/ListLayout.lua.
		ApplyListLayout(frame, buttons, count, groupName, iconSize, invertOrder)
	else -- circle
...
-- Defer cooldown updates by one frame to avoid tainting Blizzard's secure
-- event handlers. WoW 11.1+ uses "secret numbers" for cooldown values in
-- combat; touching them in an addon event handler taints the execution
-- context, which then prevents Blizzard's ActionButton_UpdateCooldown from
-- calling SetCooldown with those same secret values. By deferring to the
-- next frame via C_Timer.After(0, ...), Wise's cooldown work runs outside
