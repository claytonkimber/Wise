...
local dispatcherBtn =
	CreateFrame("Button", "WiseDispatcher", UIParent, "SecureActionButtonTemplate, SecureHandlerAttributeTemplate")
...
function Dispatcher:RegisterAction(key, actionType, actionValue, callback)
	if not key or not actionType then
		return
	end
