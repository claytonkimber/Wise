...
local STYLES, DEFAULT_STYLE = {}, "minimal"
do
	STYLES.common = {
		trackWidth = 20,
		stepperReserve = 18.5,
		...
		trackBot = {
			"t",
			"Interface/PaperDollInfoFrame/UI-Character-ScrollBar",
			tc = { 35 / 64, 62 / 64, 228 / 256, 255 / 256 },
			w = 22,
			h = 22,
		},
...
function ScrollBar:SetStepsPerPage(steps, ...)
	...
	assert(
		type(steps) == "number" and steps >= 1 and (ws == nil or type(ws) == "number" and ws >= 1),
...
function ScrollBar:Step(delta, allowAnimation)
	local d = assert(getWidgetData(self, ScrollBarData), "Invalid object type")
	assert(
		type(delta) == "number" and (allowAnimation == nil or type(allowAnimation) == "boolean"),
...
function ScrollBar:SetCoverTarget(widget, insetLeft, insetRight, insetTop, insetBottom)
	...
	assert(
		widget == nil
			or type(widget) == "table"
				and widget[0]
				and type(widget.IsObjectType) == "function"
				and widget:IsObjectType("Frame"),
		"Syntax: ScrollBar:SetCoverTarget(widget[, insetLeft, insetRight, insetTop, insetBottom])"
	)
	assert(
		insetLeft == nil and insetRight == nil and insetTop == nil and insetBottom == nil
			or type(insetLeft) == "number"
				and type(insetRight) == "number"
				and type(insetTop) == "number"
				and type(insetBottom) == "number",
		"Invalid insets"
	)
	d.coverTarget, d.coverIL, d.coverIR, d.coverIT, d.coverIB =
		widget, insetLeft or 0, insetRight or 0, insetTop or 0, insetBottom or 0
	iSB.UpdateCover(d)
end
...
function ScrollBar:SetWheelScrollTarget(widget, insetLeft, insetRight, insetTop, insetBottom)
	...
	d.wheelTarget, d.wheelIL, d.wheelIR, d.wheelIT, d.wheelIB =
		widget, insetLeft or 0, insetRight or 0, insetTop or 0, insetBottom or 0
...
function iSB.SetInteractionState(d, state)
	...
	if
		d.stepperHeld == d.StepUp and state ~= "STEP_UP_HELD"
		or d.stepperHeld == d.StepDown and state ~= "STEP_DOWN_HELD"
	then
	...
	if
		state ~= "ANIMATING_VALUE"
		and state ~= "STEP_UP_HELD"
		and state ~= "STEP_DOWN_HELD"
		and state ~= "TRACK_HELD"
	then
...
function iSB.ApplyStyle(d, style)
	...
	if stb then
		...
		d.TrackBG[4]:SetPoint(
			"BOTTOMRIGHT",
			d.TrackBG[3],
			-(stb.insetR or stb.insetH or 0),
