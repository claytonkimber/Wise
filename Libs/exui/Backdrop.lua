...
local function setBackdrop(d, info)
	if info.bgFile then
		...
		local tileH, tileV =
			not not (info.tileH == nil and info.tile or info.tileH),
			not not (info.tileV == nil and info.tile or info.tileV)
...
local function CreateBackdrop(f, info)
	assert(
		type(f) == "table"
			and type(f.IsObjectType) == "function"
			and f:IsObjectType("Frame")
