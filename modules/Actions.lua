...
local function GetOrCreateInsertIndicator(container)
	if container.slotInsertIndicator then
		return container.slotInsertIndicator
	end
...
local function GetOrCreateStateInsertIndicator(slotFrame)
	if slotFrame.stateInsertIndicator then
		return slotFrame.stateInsertIndicator
	end
...
local function UpdateDragGhostPosition(cx, cy)
	if not dragGhost or not dragGhost:IsShown() then
		return
	end
...
CleanupDrag = function(container)
	if not container then
		container = reorderDrag.container
	end
	-- Hide slot indicator
	...
	if container and container.slots then
		for _, slot in ipairs(container.slots) do
			if slot:IsShown() then
				slot:SetAlpha(1)
				if slot.stateInsertIndicator then
					slot.stateInsertIndicator:Hide()
				end
...
ReorderDrag_OnUpdate = function(self, elapsed)
	if not reorderDrag.pending and not reorderDrag.active then
		return
	end
...
FinalizeSlotReorder = function()
	...
	local srcVis = reorderDrag.sourceVisualIdx
	if not srcVis or srcVis < 1 or srcVis > #keys then
		CleanupDrag(container)
		return
	end
	...
	if group.propertyType == "CooldownWiser" then
		...
		if targetVis > srcVis then
			adjustedTarget = adjustedTarget - 1
		end
		...
		local remainingKeys = {}
		for k in pairs(group.actions) do
			remainingKeys[#remainingKeys + 1] = k
		end
				...
				if spellType == Enum.SpellBookItemType.Spell then
					...
					if sName == "Soar" or sName == "Lift Off" then
						if not seen[sName] and (not filter or string.find(string.lower(sName), filter, 1, true)) then
							...
							table.insert(
								spells,
								{ type = "spell", value = spellId, name = sName, icon = icon, category = "Skyriding" }
							)
							seen[sName] = true
						end
					end
				...
				if spellType == Enum.SpellBookItemType.Spell then
					if not IsSpellPassive(spellId) then
						...
						if sName then
							...
							-- Check keywords
							if
								string.match(sName, "^Teleport:")
								or string.match(sName, "^Portal:")
								or sName == "Dreamwalk"
								or sName == "Astral Recall"
								or sName == "Zen Pilgrimage"
								or sName == "Death Gate"
							then
								isTransport = true
							...
							if
								isTransport and (not filter or string.find(string.lower(displayName), filter, 1, true))
							then
							...
							if isKnownSlot and flyoutSpellID then
								...
								if not IsSpellPassive(actualSpellID) then
									...
									if sName then
										...
										if
											isTransport
											and (not filter or string.find(string.lower(displayName), filter, 1, true))
										then
											if not seen[displayName] then
												table.insert(items, {
													type = "spell",
		...
		for i = 1, C_ToyBox.GetNumFilteredToys() do
			...
			if itemID ~= -1 then
				...
				if name then
					...
					if
						string.match(lName, "hearthstone")
						or string.match(lName, "wormhole")
						or string.match(lName, "generator")
						or string.match(lName, "transporter")
					then
						isTransportToy = true
					else
...
function Wise:GetInterface(filter)
	...
	if not parentGroup then
		...
		for name, group in pairs(WiseDB.groups) do
			if
				(not group.isWiser or allowedWiser[name])
				and (not filter or string.find(string.lower(name), filter, 1, true))
			then
				local icon = Wise:GetActionIcon("interface", name)
				table.insert(
					items,
					{ type = "interface", value = name, name = name, icon = icon, category = "Interface" }
				)
			end
		end
		table.sort(items, function(a, b)
...
function Wise:MigrateGroupToActions(group)
	if group.migratedToActions and group.actions then
		return
	end
		...
		for _, action in ipairs(slotStates) do
			...
			if action.category then
				...
				if cat == "class" then
					...
					if cls then
						AddUniqueTag(action.visibilityEnable, "class:" .. cls)
					end
				...
				elseif cat == "character" then
					...
					if ch then
						AddUniqueTag(action.visibilityEnable, "char:" .. ch)
					end
				end
			end
		end
...
function Wise:BuildActionRecord(actionType, actionValue, category, extraData)
	...
	if category and category ~= "global" then
		resolvedCategory = category
	end
	if extraData and extraData.category then
		resolvedCategory = extraData.category
	end
	...
	if extraData and extraData.sourceSpecID then
		resolvedSpecID = extraData.sourceSpecID
	end
	if resolvedCategory == "class" then
		resolvedSpecID = nil
	end
	...
	if extraData then
		if extraData.icon then
			newAction.icon = extraData.icon
		end
		if extraData.name then
			newAction.name = extraData.name
		end
		if extraData.conditions then
			newAction.conditions = extraData.conditions
		end
	end
...
function Wise:AddAction(groupName, slotIndex, actionType, actionValue, category, extraData, insertIndex)
	...
	if Wise.DebugPrint then
		Wise:DebugPrint(
			"Wise:AddAction - TalentBuild: "
				.. tostring(newAction.talentRequirements)
				.. " | ActionValue: "
...
function Wise:ReplaceStateAction(groupName, slotIndex, stateIndex, actionType, actionValue, category, extraData)
	...
	if not group.actions[slotIndex] or not group.actions[slotIndex][stateIndex] then
		return
	end
		...
		if group.isWiser then
			if group.propertyType == "CooldownWiser" then
				...
				if type(slotIndex) == "number" and slotIndex == math.floor(slotIndex) then
					...
					if state and state.autoLoaded then
						return
					end
				end
			else
...
function Wise:GetActionName(actionType, value, extraData)
	if extraData and extraData.customName then
		return extraData.customName
	end

	-- For spells, always resolve dynamically (overrides change with spec/talents)
	if extraData and extraData.name and actionType ~= "spell" then
	...
	elseif actionType == "misc" then
		...
		if value == "leave_vehicle" then
			return "Leave Vehicle"
		end
		if value == "custom_macro" then
			return "Custom Macro"
		end
		if value == "toggle_sound" then
			return "Toggle Sound"
		end
		if value == "toggle_sfx" then
			return "Toggle Sound Effects"
		end
		if value == "vol_up" then
			return "Volume Up"
		end
		if value == "vol_down" then
			return "Volume Down"
		end
	...
	elseif actionType == "uivisibility" then
		...
		if element and state then
			...
			if element == "xpbar" then
				eName = "XP Bar"
			elseif element == "repbar" then
				eName = "Reputation Bar"
			elseif element == "micromenu" then
				eName = "Micro Menu"
			elseif element == "minimap" then
				eName = "Minimap"
			elseif element == "bags" then
				eName = "Bags Bar"
			elseif element == "friendlynameplates" then
				eName = "Friendly Nameplates"
			elseif element == "enemynameplates" then
				eName = "Enemy Nameplates"
			end
...
function Wise:GetActionIcon(actionType, value, extraData)
	-- For spells, always resolve dynamically (overrides change with spec/talents)
	if extraData and extraData.icon and actionType ~= "spell" then
		return extraData.icon
	end
	...
	elseif actionType == "uivisibility" then
		...
		elseif element == "editmode" then
			texture = "Interface\\Icons\\INV_Misc_EngGizmos_17"
		elseif element == "bags" then
			texture = "Interface\\Icons\\INV_Misc_Bag_08"
		elseif element == "xpbar" then
		...
		elseif element == "chat" then
			texture = "Interface\\Icons\\Spell_Holy_Silence"
		elseif element == "objectives" then
			texture = "Interface\\Icons\\INV_Misc_Book_07"
		elseif element == "player" then
			texture = "Interface\\Icons\\Achievement_Character_Human_Male"
		...
		elseif element == "quickchat" then
			texture = "Interface\\Icons\\INV_Misc_ChatBubble"
		elseif element == "friendlynameplates" then
			texture = "Interface\\Icons\\Achievement_ReputationCity_Friendly"
		elseif element == "enemynameplates" then
	...
	elseif actionType == "misc" then
		...
		if value == "toggle_sound" then
			texture = "Interface\\AddOns\\Wise\\Media\\SoundToggle.tga"
		end
		if value == "toggle_sfx" then
			texture = "Interface\\AddOns\\Wise\\Media\\SFXToggle.tga"
		end
		if value == "vol_up" then
			texture = "Interface\\AddOns\\Wise\\Media\\VolUp.tga"
		end
		if value == "vol_down" then
			texture = "Interface\\AddOns\\Wise\\Media\\VolDown.tga"
		end
		if type(value) == "string" and string.sub(value, 1, 5) == "spec_" then
...
function Wise:IsActionKnown(actionType, value)
					...
					if (not lineSpecID) or (lineSpecID == currentSpecID) then
						...
						for j = 1, count do
							...
							local spellType, bookSpellID =
								C_SpellBook.GetSpellBookItemType(index, Enum.SpellBookSpellBank.Player)
							if spellType == Enum.SpellBookItemType.Spell then
								-- Check both direct match and override match (e.g. Maul -> Raze)
								local overrideID = Wise:GetOverrideSpellID(bookSpellID)
								if
									bookSpellID == spellID
									or overrideID == spellID
									or (
										storedOverride
	...
	elseif actionType == "toy" then
		...
		if PlayerHasToy and not PlayerHasToy(toyID) then
			return false
		end
	...
	elseif actionType == "equipmentset" then
		if C_EquipmentSet then
			return C_EquipmentSet.GetEquipmentSetID(value) ~= nil
		end
...
function Wise:SetCastReadout(f, actionType, value, groupName)
	...
	elseif WiseDB and WiseDB.settings then
		if WiseDB.settings.showGCD ~= nil then
			showGCD = WiseDB.settings.showGCD
		end
	end
...
function Wise:CreateEmbeddedPicker(parent)
	...
	for _, cat in ipairs(categories) do
		...
		btn:SetScript("OnEnter", function()
			catText:SetTextColor(1, 0.8, 0)
		end)
		btn:SetScript("OnLeave", function()
			catText:SetTextColor(1, 1, 1)
		end)
	...
	ep.CategoryBtn:SetScript("OnClick", function()
		if ep.CategoryMenu:IsShown() then
			ep.CategoryMenu:Hide()
		else
			ep.CategoryMenu:Show()
		end
	end)
	...
	local function UpdateSearchInstructions(self)
		...
		if not instr then
			for _, region in ipairs({ self:GetRegions() }) do
				if
					region:IsObjectType("FontString") and (region:GetText() == SEARCH or region:GetText() == "Search")
				then
					instr = region
...
function Wise:PickerSelectCategory(catName)
	...
	if ep.Scroll then
		ep.Scroll:SetVerticalScroll(0)
	end
end

function Wise:PickerRefresh(filter)
	local ep = Wise.EmbeddedPicker
	if not ep or not ep.Content then
		return
	end
			...
			btn:SetScript("OnClick", function(self)
				if Wise.PickerCallback then
					...
					if self.data.icon then
						extra.icon = self.data.icon
					end
					if self.data.name then
						extra.name = self.data.name
					end
					if self.data.category then
						extra.category = self.data.category
					end
					if self.data.sourceSpecID then
						extra.sourceSpecID = self.data.sourceSpecID
					end
					if self.data.conditions then
						extra.conditions = self.data.conditions
					end
					if self.data.addonName then
						extra.addonName = self.data.addonName
					end
...
function Wise:GetSpell(filter)
	...
	for i = 1, numSkillLines do
		...
		if info then
			...
			if Wise.PickerSpellFilter == displayCategory then
				showSpell = true
			end
					...
					if spellType == Enum.SpellBookItemType.Spell then
						if not IsSpellPassive(spellId) then
							...
							if
								...
								and (
									not filter
									or string.find(string.lower(name), filter, 1, true)
									or string.find(string.lower(description), filter, 1, true)
								)
							then
								if not seen[name] then
									table.insert(spells, {
								...
								if isKnownSlot and flyoutSpellID then
									...
									if not IsSpellPassive(actualSpellID) then
										...
										if
											...
											and (
												not filter
												or string.find(string.lower(sName), filter, 1, true)
												or string.find(string.lower(description), filter, 1, true)
											)
										then
	...
	if Wise.PickerSpellFilter == "All" then
		...
		if not seen["Assist"] then
			...
			if info then
				table.insert(
					spells,
					{ type = "spell", value = "Assist", name = info.name, icon = info.iconID, category = "global" }
				)
				seen["Assist"] = true
			end
		end
		...
		if not seen["Attack"] then
			...
			if info then
				table.insert(
					spells,
					{ type = "spell", value = "Attack", name = info.name, icon = info.iconID, category = "global" }
				)
				seen["Attack"] = true
			end
		end
		...
		if not seen["Single-Button Assistant"] then
			...
			if info then
				table.insert(
					spells,
					{ type = "spell", value = info.spellID, name = info.name, icon = info.iconID, category = "global" }
				)
				seen[info.name] = true
			end
		end
...
function Wise:GetMounts(filter, favoritesOnly)
	...
	for i = 1, 4200 do
		local name, spellID, icon, active, isUsable, sourceType, isFavorite, isFactionSpecific, faction, hideOnChar, isCollected, mountID =
			C_MountJournal.GetMountInfoByID(i)

		if name and isCollected then
			local match = true
			if favoritesOnly and not isFavorite then
				match = false
			end
...
function Wise:GetUIVisibility(filter)
	...
	local elements = {
		...
		{ id = "quickchat", name = "Quick Chat", icon = "Interface\\Icons\\INV_Misc_ChatBubble" },
		{
			...
			icon = "Interface\\Icons\\Achievement_ReputationCity_Friendly",
		},
				...
				if showMain then
					...
					table.insert(items, {
						...
						tooltipFunc = function()
							GameTooltip:SetText(name .. " (Toggle)")
						end,
					})
					seen[name] = true
				end
				...
				if numAbilities and spellOffset then
					for j = 1, numAbilities do
						...
						local spellType, spellID =
							C_SpellBook.GetSpellBookItemType(slotIndex, Enum.SpellBookSpellBank.Player)
						if spellType == Enum.SpellBookItemType.Spell then
							local sName = C_Spell.GetSpellName(spellID)
							local sIcon = C_Spell.GetSpellTexture(spellID)
								...
								for s = 1, numSlots do
									...
									if isKnownSlot and flyoutSpellID then
										...
										if not IsSpellPassive(actualSpellID) then
											...
											if
												sName
												and (not filter or string.find(string.lower(sName), filter, 1, true))
											then
												if not seen[sName] then
													table.insert(items, {
														type = "spell",
...
function Wise:GetSpecialActionbars(filter)
	...
	for _, m in ipairs(specialActionMisc) do
		if not filter or string.find(string.lower(m.name), filter, 1, true) then
			table.insert(
				items,
				{ type = "misc", value = m.val, name = m.name, icon = m.icon, category = "Special Action bars" }
			)
		end
	end
...
function Wise:GetMiscellaneous(filter)
	...
	if not filter or string.find("empty slot", filter, 1, true) then
		table.insert(
			items,
			{ type = "empty", value = nil, name = "Empty Slot", icon = "Interface\\PaperDoll\\UI-Backpack-EmptySlot" }
		)
	end
...
function Wise:RefreshActionsView(container)
	...
	for _, sIdx in ipairs(keys) do
		...
		slotFrame:SetScript("OnMouseDown", function(self, button)
			...
			if reorderDrag.active or reorderDrag.pending then
				return
			end
			local grp = WiseDB.groups[groupName]
			if grp and grp.isLocked then
				return
			end
		...
		for _, b in ipairs(slotFrame.ActionButtons) do
			b:Hide()
		end
		...
		for aIdx, action in ipairs(actions) do
			...
			if Wise:ShouldShowAction(action) then
				...
				if
					...
					and action.value:sub(1, 12) == "addon_magic_"
				then
					local amIdx = tonumber(action.value:sub(13))
					if amIdx and WiseDB.addonMagicSlots and WiseDB.addonMagicSlots[amIdx] then
						local count = WiseDB.addonMagicSlots[amIdx].addons and #WiseDB.addonMagicSlots[amIdx].addons
							or 0
						if count == 0 then
	...
	-- Hide unused slots
	for k = slotIndex + 1, #container.slots do
		container.slots[k]:Hide()
	end
...
function Wise:CreateIconPicker(parent)
	...
	if Wise.exUI then
		...
		if isd.closeButton then
			isd.closeButton:Hide()
		end
