...
local function ApplyOffsetFromPopup()
	if not selectionPopup or not selectedEditFrame or not selectedEditName then
		return
	end
...
local function CreateSelectionPopup()
	...
	popup.xBox:SetScript("OnEscapePressed", function(self)
		self:ClearFocus()
	end)
	...
	popup.yBox:SetScript("OnEscapePressed", function(self)
		self:ClearFocus()
	end)
...
local function SyncPopupAfterDrag(f, name)
	if not selectionPopup or not selectionPopup:IsShown() then
		return
	end
	if selectedEditFrame ~= f then
		return
	end
	local group = WiseDB.groups[name]
	if not group or not group.anchor then
		return
	end
...
function Wise:SetFrameEditMode(f, name, enabled)
	...
	if enabled then
		...
		if fLeft and fRight and fTop and fBottom and f.buttons then
			...
			for _, btn in ipairs(f.buttons) do
				if btn:IsShown() then
						...
						for _, r in ipairs(regions) do
							if r and r:IsShown() then
								...
								if valid then
									...
									if rL and (not bLeft or rL < bLeft) then
										bLeft = rL
									end
									if rR and (not bRight or rR > bRight) then
										bRight = rR
									end
									if rT and (not bTop or rT > bTop) then
										bTop = rT
									end
									if rB and (not bBottom or rB < bBottom) then
										bBottom = rB
									end
								end
							end
					...
					if bLeft and (not minLeft or bLeft < minLeft) then
						minLeft = bLeft
					end
					if bRight and (not maxRight or bRight > maxRight) then
						maxRight = bRight
					end
					if bTop and (not maxTop or bTop > maxTop) then
						maxTop = bTop
					end
					if bBottom and (not minBottom or bBottom < minBottom) then
						minBottom = bBottom
					end
				end
			end
		...
		f:SetScript("OnDragStop", function(self)
			...
			local snapped = false
			if
				not Wise.wiseOnlyEditMode
				and EditModeManagerFrame
				and EditModeManagerFrame.Grid
...
local function CreateExitEditModeFrame()
	if Wise.ExitEditModeFrame then
		return Wise.ExitEditModeFrame
	end
