...
function Wise:SetTab(viewName)
	...
	if Wise.configuringSlot and viewName ~= "Editor" then
		...
		if f.ConfiguratorHost then
			f.ConfiguratorHost:Hide()
		end
		if f.ConditionPickerHost then
			f.ConditionPickerHost:Hide()
		end
		-- Restore sidebar/middle content
		if f.Sidebar.AddBtn then
			f.Sidebar.AddBtn:Show()
		end
		if f.Sidebar.Scroll then
			f.Sidebar.Scroll:Show()
		end
		if f.Sidebar.Content then
			f.Sidebar.Content:Show()
		end
		if f.Middle.Content then
			f.Middle.Content:Show()
		end
		if f.Middle.ScrollFrame then
			f.Middle.ScrollFrame:Show()
		end
		if f.Middle.AddSlotBtn then
			f.Middle.AddSlotBtn:Show()
		end
	end
...
function Wise:RefreshGroupList()
	...
	if container.headers then
		for _, hdr in ipairs(container.headers) do
			hdr:Hide()
		end
	end
	...
	for _, name in ipairs(customGroups) do
		...
		btn.lockBtn:SetScript("OnEnter", function(self)
			...
			GameTooltip:AddLine(
				"Prevents all changes to this interface, including drag & drop and keybinds.",
				0.8,
				0.8,
				0.8,
				true
		...
		btn.lockBtn:SetScript("OnLeave", function(self)
			GameTooltip:Hide()
		end)
	...
	if Wise.BAR_COPY_TEMPLATE then
		...
		if barBtn.lockBtn then
			barBtn.lockBtn:Hide()
		end
		if barBtn.errorIcon then
			barBtn.errorIcon:Hide()
		end
	...
	if smartBtn.lockBtn then
		smartBtn.lockBtn:Hide()
	end
	if smartBtn.errorIcon then
		smartBtn.errorIcon:Hide()
	end
	...
	if isSyndicator then
		...
		smartBtn:SetScript("OnLeave", function(self)
			GameTooltip:Hide()
		end)
