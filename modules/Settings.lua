...
function Wise:PopulateSettingsView(panel)
	...
	addToContent = function(c, child, x, y)
		child:SetParent(c)
		child:ClearAllPoints()
	...
	if LSM then
		...
		table.sort(validFonts, function(a, b)
	...
	showIntKb:SetScript("OnClick", function(self)
		...
		C_Timer.After(0.1, function()
			if not InCombatLockdown() then
				...
				if Wise.frames then
					for groupName, f in pairs(Wise.frames) do
						local _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, showInterfaceKeybind =
	...
	local positions = {
		{ val = "TOPLEFT", text = "TL" },
		{ val = "TOP", text = "T" },
		{ val = "TOPRIGHT", text = "TR" },
		{ val = "LEFT", text = "L" },
		{ val = "CENTER", text = "C" },
		{ val = "RIGHT", text = "R" },
		{ val = "BOTTOMLEFT", text = "BL" },
		{ val = "BOTTOM", text = "B" },
		{ val = "BOTTOMRIGHT", text = "BR" },
	}
	local startY = ry
	...
	if WiseDB.settings.showBuffStacks == nil then
		WiseDB.settings.showBuffStacks = true
	end
	...
	if WiseDB.settings.showDebuffs == nil then
		WiseDB.settings.showDebuffs = true
	end
	...
	enabledOpacitySlider:SetScript("OnValueChanged", function(self, val)
		...
		C_Timer.After(0.1, function()
			if not InCombatLockdown() then
				for gname in pairs(WiseDB.groups) do
					Wise:UpdateGroupDisplay(gname)
				end
			end
		end)
	end)
	...
	enabledOpacityMinusBtn:SetScript("OnClick", function()
		...
		if v >= min then
			enabledOpacitySlider:SetValue(v)
		end
	end)
	...
	disabledOpacityMinusBtn:SetScript("OnClick", function()
		...
		if v >= min then
			disabledOpacitySlider:SetValue(v)
		end
	end)
