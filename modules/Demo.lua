...
local function FindControl(textPattern, controlType)
	if not Wise.OptionsFrame or not Wise.OptionsFrame.Right or not Wise.OptionsFrame.Right.Content then
		return nil
	end
	...
	for i, ctrl in ipairs(controls) do
		if ctrl:IsShown() then
			...
			if not text and ctrl.text and ctrl.text.GetText then
				text = ctrl.text:GetText()
			end
...
local function FindSidebarButton(namePattern)
	...
	if not f or not f.Sidebar or not f.Sidebar.Content then
		return nil
	end
...
local function FindPickerButton(namePattern)
	...
	if not picker or not picker.SpellFilterButtons then
		return nil
	end
...
local function ClearAllGlows()
	if not Wise.HideOverlayGlow then
		return
	end
...
local function EnsureGroupSelected()
	if not demoActive or not lastCreatedGroup then
		return
	end
...
local Steps = {
			...
			if Wise.OptionsFrame and Wise.OptionsFrame.Views and Wise.OptionsFrame.Views.Settings then
				...
				if content.children then
					for _, ctrl in ipairs(content.children) do
						if
							ctrl:IsObjectType("CheckButton")
							and ctrl.text
							and ctrl.text:GetText() == "Show Minimap Button"
						then
							return ctrl
						end
					end
	...
	-- 4. Close the window
	{
		...
		buttonStyle = HelpTip.ButtonStyle.None,
		onEnter = function()
			if Wise.OptionsFrame.CloseButton then
				Glow(Wise.OptionsFrame.CloseButton)
			end
		end,
		onExit = function()
			if Wise.OptionsFrame.CloseButton then
				Unglow(Wise.OptionsFrame.CloseButton)
			end
		end,
		check = function()
	...
	-- 7. Name it and click Create
	{
		...
		target = function()
			return _G["StaticPopup1"] or UIParent
		end,
		point = HelpTip.Point.TopEdgeCenter,
		buttonStyle = HelpTip.ButtonStyle.None,
		onEnter = function()
...
function Wise.ToggleOptions(self)
	if origToggleOptions then
		origToggleOptions(self)
	end
